...
type Props = {
  cancelJoin: () => void; // Closes confirmAddGRoupChat Modal
...
const ConfirmAddGroupChat = (props: Props) => {
  const {sendMessage} = useBetterSocket();
    ...
    const handleJoin =(e: React.MouseEvent) => {
      sendMessage("JOIN_ROOM", {roomId: props.roomId});
      props.confirmJoin();
    }
      ...
      <div className="join-group-chat" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="join-prompt">
          ...
          <div className="join-buttons">
            <button onClick={handleJoin} className="join-confirm">Join</button>
            <button onClick={handleCancel} className="join-cancel">Cancel</button>
          </div>
        </div>
      </div>
    </Modal>
